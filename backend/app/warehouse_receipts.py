...
def list_receipts(db, team_id, user, *, record_filters=None, query=None, material_type=None, page=1, page_size=20):
    require_warehouse(stock.require_team(db, team_id))
    filters = [MaterialTransfer.next_team_id == team_id, MaterialTransfer.entry_kind == "warehouse_receipt"]
    ...
    items = db.scalars(select(MaterialTransfer).where(*filters)
        .order_by(MaterialTransfer.created_at.desc(), MaterialTransfer.id.desc())
