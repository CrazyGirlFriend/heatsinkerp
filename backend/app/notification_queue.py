            ...
            try:
                ...
                async with queue.iterator() as messages:
                    ...
                    async for message in messages:
                        ...
                        # IDs generated by this service are safe diagnostic metadata.
                        safe_id = message.message_id
                        if not (
                            isinstance(safe_id, str)
                            and len(safe_id) == 32
                            and all(c in "0123456789abcdef" for c in safe_id)
                        ):
                            safe_id = None
                        record(
...
class NotificationService:
    ...
    async def send_batch(self):
        ...
        claim_ms = round((monotonic() - started) * 1000, 2)
        for message_id, payload, attempts in rows:
            record(
                "notification.claimed", message_id=message_id, attempts=attempts, claim_ms=claim_ms
            )
            error = None
            started = monotonic()
            try:
