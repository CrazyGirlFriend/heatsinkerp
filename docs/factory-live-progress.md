...
- 材质轮播专项：后端50项、前端54项、TypeScript和生产构建通过。独立Chrome实测2K稳定换页间隔约2秒、翻页按钮位置不跳动，左右转料持续移动；390px窄屏换页区保持等高，无横向溢出。截图为 `output/playwright/robot-materials-2k.png`、`robot-materials-page-2k.png` 和 `robot-materials-mobile.png`。本地后端已重启加载新字段，未提交、推送或部署。
...
- 发布时需一并更新前后端；接口为新增字段，不需数据库迁移。详细画面对照见根目录 `design-qa.md`。
