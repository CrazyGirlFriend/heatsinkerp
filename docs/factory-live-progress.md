...
## 2026-09-18：扣减口径与独立批次已部署

`6fdf069` / `20260918T045500Z` 已上线。下述对外出库提交扣减修复生效；首页、机器人与班组库存统一。待接收和近期记录按每个实际批次计数，不再按旧 CK 整单合并。上线验收全厂在库 17,298 件、内部在途 1,648 件；六种具体材质件数/重量及八班组待接收记录与 MySQL 一致，三路实时推送正常。原数据保留，未导入本地演示物料；详见 [服务器部署记录](server-deployment.md)。以下保留各开发阶段的原始记录。

## 2026-09-17：对外出库扣减口径修复（本地，待部署）

库房对外出库、检验发货改为提交即扣减当前库存，确认仅锁定单据、不重复扣库，未确认单作废返还库存。大屏材质合计、全厂首页、班组明细、材质及库龄统计共用同一余额；实时推送同步更新。转出趋势统一按提交日期统计；内部在途仍只包含班组间待接收量。正常可用库存仍不包含废料。无数据库迁移，不改写历史单据。

验证：后端全量 306 项、前端全量 440 项通过，最终说明文字调整后相关 45 项复验及 TypeScript/生产构建通过。覆盖两类对外出库的提交、幂等重试、改单、作废返还、整批/逐条确认、跨日转出统计、废料及纯件数/纯重量出库，验证首页与大屏的提交后 SSE 推送。

本地预览服务已载入修复；通过实际 HTTP 接口与两路 SSE 核对八班组数据，并从原始收发/丢失记录独立重算：当前在库 17,310 件 / 1,731 kg = 正常可用 17,298 件 / 1,729.8 kg + 废料 12 件 / 1.2 kg。该废料为本地测试记录，不能与线上旧数据直接比较。本轮没有提交、推送或部署，没有修改线上业务记录，未使用浏览器。

以下为历史版本记录，其对外确认后扣库口径已由本次修复替代。
