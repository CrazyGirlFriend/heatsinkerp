...
import { BarChart, LineChart, PieChart, CustomChart, ScatterChart } from 'echarts/charts'
...
function resize() {
  ...
  // An unnecessary resize completes ECharts' entrance clip immediately.
  if (root.value && (chart.getWidth() !== root.value.clientWidth || chart.getHeight() !== root.value.clientHeight)) chart.resize()
}
function focusPoint() {
...
onMounted(() => { observer = typeof ResizeObserver !== 'undefined' ? new ResizeObserver(resize) : undefined; if (root.value) observer?.observe(root.value); render() })
