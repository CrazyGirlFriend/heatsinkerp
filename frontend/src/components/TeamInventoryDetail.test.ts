...
beforeEach(() => { vi.spyOn(teamMaterialApi, 'inventorySources').mockResolvedValue({ items: [source(11, 5), source(12, 0, 'CK20260917000001')], total: 21, page: 1, page_size: 10 }) })
...
describe('warehouse source detail', () => {
  it('shows the upstream team on a workshop group and forwards losses from the actual batch', async () => {
    ...
    await wrapper.setProps({ group: warehouseFixture({ receipt_source: 'internal', source_name: '轧制', source_team_id: 2 }) }); await flushPromises()
