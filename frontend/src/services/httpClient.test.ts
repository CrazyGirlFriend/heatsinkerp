...
describe('shared Axios client', () => {
  it('uses the shared base URL, credentials, JSON headers and current authorization', async () => {
    localStorage.setItem(
      ...
      JSON.stringify({ access_token: 'shared-token', token_type: 'Bearer' }),
    )
    responseBody = { ok: true }
    await expect(
      httpRequest('/material-transfers', {
        method: 'POST',
        body: JSON.stringify({ serial_no: 'SERIAL-1' }),
      }),
    ).resolves.toEqual({ ok: true })

    const config = adapter.mock.calls[0]![0]
    expect(config).toMatchObject({
      url: '/material-transfers',
      method: 'post',
      baseURL: '/api',
  ...
  it('locks site access on 423 while preserving account authentication', async () => {
    ...
    await expect(httpRequest('/material-transfers')).rejects.toMatchObject({
      status: 423,
      body: responseBody,
    })
  ...
  it('invalidates a stale session on 401 except for a rejected login request', async () => {
    ...
    await expect(
      httpRequest('/auth/login', { method: 'POST', skipAuthExpiry: true }),
...
it('retains the server request ID without exposing response content in diagnostics', async () => {
  const warning = vi.spyOn(console, 'warn').mockImplementation(() => {})
  try {
    responseStatus = 503
    responseBody = { detail: 'PRIVATE-RESPONSE' }
    await expect(httpRequest('/private?token=PRIVATE-URL')).rejects.toMatchObject({
      status: 503,
