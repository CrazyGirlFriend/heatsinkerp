...
export const warehouseColumns = [
  { key: 'material_name', label: '材质', width: 180, defaultVisible: true, format: (row: TeamInventoryRow) => row.material_name || '—' },
  { key: 'transfer_specification', label: '规格', width: 180, defaultVisible: true, format: (row: TeamInventoryRow) => row.transfer_specification || '—' },
  { key: 'material_type', label: '物料类型', width: 150, defaultVisible: true, format: (row: TeamInventoryRow) => materialTypeLabel(row.material_type || null) },
...
export const warehouseSerialColumn = { key: 'serial_no', label: '流水号', width: 180, format: (row: TeamInventoryRow) => row.serial_no } as const
