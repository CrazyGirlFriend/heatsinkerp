...
import { FullScreen, Search, Upload } from '@element-plus/icons-vue'
...
const tabValues = computed(() => ['overview', 'serials', 'materials', 'pending', 'stock', 'outgoing', 'losses', ...(isWarehouse.value ? ['receipts'] : [])])
const queryText = (key: string) => typeof route.query[key] === 'string' ? String(route.query[key]) : ''
const tab = computed(() => tabValues.value.includes(queryText('tab')) ? queryText('tab') : queryText('direction') === 'outgoing' ? 'outgoing' : queryText('direction') === 'incoming' ? queryText('status') === 'received' ? 'stock' : 'pending' : 'serials')
...
const availabilityDraft = ref('available')
...
const selectedRows = computed(() => rows.value.filter(row => checked.value.includes(String(row.transfer.id))))
const selectableRows = computed(() => rows.value.filter(stockAvailable))
const allChecked = computed(() => selectableRows.value.length > 0 && selectableRows.value.every(row => checked.value.includes(String(row.transfer.id))))
...
function selectTab(value: string) { void router.push({ path: route.path, query: value === 'serials' ? {} : { tab: value } }) }
function applyFilters(nextPage = 1, size = pageSize.value) {
  checked.value = []
  void router.replace({ path: route.path, query: { tab: tab.value, ...(dateDraft.value.from ? { date_from: dateDraft.value.from } : {}), ...(dateDraft.value.to ? { date_to: dateDraft.value.to } : {}), ...(urgentDraft.value ? { urgent_only: 'true' } : {}), ...(queryDraft.value.trim() ? { query: queryDraft.value.trim() } : {}), ...(['stock', 'receipts'].includes(tab.value) ? { ...(materialDraft.value ? { material_type: materialDraft.value } : {}), ...(availabilityDraft.value === 'all' ? { availability: 'all' } : {}) } : {}), ...(tab.value === 'outgoing' ? { ...(kindDraft.value ? { entry_kind: kindDraft.value } : {}), ...(statusDraft.value ? { status: statusDraft.value } : {}), ...(!isExternalEntryKind(kindDraft.value) && nextTeamDraft.value ? { next_team_id: String(nextTeamDraft.value) } : {}) } : {}), ...(nextPage > 1 ? { page: String(nextPage) } : {}), ...(size !== 10 ? { page_size: String(size) } : {}) } })
}
...
function toggleCheck(row: StockBatch, value: string | number | boolean) { const id = String(row.transfer.id); checked.value = value ? [...new Set([...checked.value, id])] : checked.value.filter(value => value !== id) }
function toggleAll(value: string | number | boolean) { checked.value = value ? selectableRows.value.map(row => String(row.transfer.id)) : [] }
...
async function loadView(refreshWarehouse: unknown = false, background = false) {
  ...
  if (!scopeReady.value) { overview.value = null; rows.value = []; pending.value = []; outgoing.value = []; losses.value = []; receipts.value = []; total.value = 0; loading.value = false; return }
  if (!background) { loading.value = true; loadError.value = ''; overviewError.value = ''; checked.value = [] }
  syncError.value = ''
  ...
  const refreshRequests = refreshWarehouse === true && isWarehouse.value ? [
    ...(currentTab !== 'stock' ? [teamMaterialApi.stock(id, { page: 1, page_size: 20, availability: 'available' }).then(result => { if (current === version) rows.value = result.items })] : []),
  ...
  try {
    if (currentTab === 'stock') { const result = await teamMaterialApi.stock(id, { ...params, material_type: materialType, availability: queryText('availability') === 'all' ? 'all' : 'available' }); if (current === version) { rows.value = result.items; total.value = result.total } }
    else if (currentTab === 'pending') { const result = await materialTransferApi.list({ ...params, team_id: id, direction: 'incoming', status: 'pending' }); if (current === version) { pending.value = result.items; total.value = result.total } }
  ...
  } catch (error) { if (current === version) { if (background) syncError.value = '数据更新失败，保留上次结果，请刷新重试。'; else { rows.value = []; pending.value = []; outgoing.value = []; losses.value = []; receipts.value = []; total.value = 0; loadError.value = error instanceof Error ? error.message : '物料记录加载失败' } } }
  await Promise.all([balanceRequest, ...refreshRequests])
  if (current === version) loading.value = false
}

watch([() => ['overview', 'serials', 'materials'].includes(tab.value) ? `${route.path}:${tab.value}` : route.fullPath, scopeReady], () => {
  closeDetails(); checked.value = [];  rows.value = []; pending.value = []; outgoing.value = []; losses.value = []; receipts.value = []; overview.value = null
  dateDraft.value = { from: queryText('date_from'), to: queryText('date_to') }; urgentDraft.value = queryText('urgent_only') === 'true'
  queryDraft.value = queryText('query'); materialDraft.value = materialTypeOptions.find(option => option.value === queryText('material_type'))?.value || ''; availabilityDraft.value = queryText('availability') === 'all' ? 'all' : 'available'; statusDraft.value = Object.keys(dispatchStatusLabels).includes(queryText('status')) ? queryText('status') as DispatchStatus : ''; nextTeamDraft.value = queryText('next_team_id'); kindDraft.value = dispatchKinds.find(kind => kind === queryText('entry_kind')) || ''
  void loadView()
}, { immediate: true })
  ...
  <TeamWorkspaceShell :title="title" :model-value="tab" :warehouse="isWarehouse" :pending-count="overview?.pending_incoming.count" :class="{ 'team-workspace--docked': docked && detailOpen }" @update:model-value="selectTab">
    ...
    <div ref="container" class="team-material-content">
      ...
      <template v-else>
        ...
        <template v-if="['overview', 'serials', 'materials'].includes(tab)">
          ...
          <TeamSerialOverview v-else-if="overview && tab === 'serials'" :key="teamId" :team-id="teamId" :overview="overview" @changed="loadView" />
          <TeamMaterialOverviewPanel v-else-if="overview" :overview="overview" @filter="router.push({ path: route.path, query: { tab: 'serials', material_name: $event, filter_label: `库存材质：${$event}` } })" />
        </template>
        <template v-else>
          <div class="team-list-layout" :class="{ 'team-list-layout--detail': docked && detailOpen }">
            <section class="team-list-panel">
              <div class="list-toolbar" :class="{ 'list-toolbar--pending': tab === 'pending' }">
                ...
                <ElSelect v-if="tab === 'stock' || tab === 'receipts'" v-model="materialDraft" aria-label="物料类型筛选" placeholder="全部类型" clearable @change="applyFilters()"><ElOption v-for="type in materialTypeOptions" :key="type.value" :value="type.value" :label="type.label" /></ElSelect>
                <ElSelect v-if="tab === 'stock'" v-model="availabilityDraft" aria-label="余量筛选" @change="applyFilters()"><ElOption value="available" label="有可用库存" /><ElOption value="all" label="全部来源批次" /></ElSelect>
                ...
                <RecordDateFilter v-model="dateDraft" :label="tab === 'stock' ? '接收日期' : tab === 'receipts' ? '入库日期' : '登记日期'" @update:model-value="applyFilters()" />
              ...
              <div v-if="tab === 'stock' && canWrite" class="selection-bar"><span>选择物料出库<span v-if="checked.length"> · 已选 {{ checked.length }} 批</span></span><ElButton type="primary" :icon="Upload" :disabled="!checked.length || loading" @click="openAction('dispatch', selectedRows)">批量出库<span v-if="checked.length">（{{ checked.length }}）</span></ElButton></div>
              ...
              <StatePanel v-else-if="!total" state="empty" :title="tab === 'pending' ? '暂无待接收来料' : tab === 'stock' ? '暂无符合条件的已接收物料' : tab === 'outgoing' ? '暂无出库记录' : tab === 'receipts' ? '暂无手工入库记录' : '暂无丢失记录'" :description="tab === 'stock' ? '确认接收后，可在这里按来源批次出库或登记丢失。' : '可以调整搜索条件或刷新记录。'" />
              <div v-else class="team-table-scroll">
                <ElTable v-if="tab === 'stock'" :data="rows" class="business-table team-table" row-key="transfer.id">
                  <ElTableColumn v-if="canWrite" width="42" class-name="stock-selection-column">
                    <template #header><ElCheckbox aria-label="选择本页可用批次" :model-value="allChecked" :indeterminate="checked.length > 0 && !allChecked" :disabled="!selectableRows.length" @change="toggleAll" /></template>
                    <template #default="{ row }"><ElCheckbox :aria-label="'选择 ' + row.transfer.batch_no" :model-value="checked.includes(String(row.transfer.id))" :disabled="!stockAvailable(asStock(row))" @change="toggleCheck(asStock(row), $event)" /></template>
                  </ElTableColumn>
                  <ElTableColumn label="来源批次 / 流水号" min-width="240"><template #default="{ row }"><button class="batch-link" @click="openDetail(row.transfer)">{{ row.transfer.batch_no }}</button><small class="cell-secondary">{{ row.transfer.serial_no }}<SerialUrgencyBadge :urgency="row.transfer.urgency" /></small></template></ElTableColumn>
                  <ElTableColumn label="材质 / 类型" min-width="140"><template #default="{ row }">{{ row.transfer.material_name || '—' }}<small class="cell-secondary">{{ materialTypeLabel(row.transfer.material_type) }}</small></template></ElTableColumn>
                  <ElTableColumn label="可用库存" min-width="150"><template #default="{ row }"><MaterialAmount :quantity="row.available_quantity" :weight="row.available_weight" /></template></ElTableColumn>
                  ...
                  <ElTableColumn label="累计丢失" min-width="130"><template #default="{ row }"><MaterialAmount :quantity="row.lost_quantity" :weight="row.lost_weight" /></template></ElTableColumn>
                  <ElTableColumn label="来源班组" min-width="132" show-overflow-tooltip><template #default="{ row }">{{ row.transfer.source_team.name }}</template></ElTableColumn>
                  <ElTableColumn label="操作" width="145" fixed="right"><template #default="{ row }"><div class="row-actions"><template v-if="canWrite"><ElButton link type="primary" :disabled="!stockAvailable(asStock(row))" @click="openAction('dispatch', [asStock(row)])">出库</ElButton><ElButton link :disabled="!stockAvailable(asStock(row))" @click="openAction('loss', [asStock(row)])">登记丢失</ElButton></template><ElButton v-else link type="primary" @click="openDetail(row.transfer)">查看明细</ElButton></div></template></ElTableColumn>
                </ElTable>
                <ElTable v-else-if="tab === 'pending'" :data="pending" class="business-table team-table" row-key="id">
              ...
              <footer v-if="!loading && !loadError" class="table-footer"><span>共 {{ total }} {{ tab === 'stock' ? '个来源批次' : '条记录' }}</span><ElPagination :current-page="page" :page-size="pageSize" :page-sizes="[10, 20, 50, 100]" :total="total" layout="sizes, prev, pager, next" @current-change="applyFilters($event)" @size-change="applyFilters(1, $event)" /></footer>
            </section>
...
.list-toolbar, .scanner-error, .selection-bar, .table-footer { flex-shrink: 0; }
...
.selection-bar { display: flex; justify-content: space-between; align-items: center; gap: 10px; padding: 0 12px 8px; font-size: 12px; color: var(--subtle); }
