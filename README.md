...
批量出库采用“一批一码”：Code 128 编码整批 `CK…` 批次号，扫码查看全部物料明细并一次确认，打印为单联 A4 汇总单。明细保留 TL 标识和来源关联，不再逐行打印条码；旧 TL 条码、独立手工转料及入库单仍兼容。蓝牙扫码枪按 HID 键盘输入编号并回车，不需要摄像头、蓝牙 SDK 或厂商插件。流水号表示工件信息，批次号表示本次实际转料。
...
- 主系统流水号资料查询、权威快照入库接口已在本地实现，尚未部署或与真实主系统联调；前端入口和全字段整批打印待配套接入。对接方可直接查看 [接口规范](docs/main-system-integration.md)。
...
- 每班组包含独立的数据分析、流水号台账、材质归类、待接收、库存明细、出库记录、丢失记录。图表与台账分开，普通业务列表默认 10 条，可选 20/50/100 条并纵向滚动；单据详情与打印使用表格。当前视觉规则见 [界面一致性约定](docs/ui-style-guide.md)。
- 库房另有手工入库与入库记录：库房班组长手工填写流水号、材质、物料类型、件数、重量和入库说明，提交后直接入库，无需公司主系统或虚拟上序。已入库物料可继续分批/批量出库或登记丢失。
- 已接收批次支持单批/向同一下序批量出库和带原因的丢失登记。整批一个 CK 条码，各行保留来源；下序一次核对确认全部待接收明细，事务行锁和幂等避免超扣、重复提交及部分写入。
- 内部转料提交即从上序库存扣除，待接收期间单独计入在途，下序确认后入下序库存，上序不重复扣减。作废未接收单恢复上序库存；班组汇总、材质和流水号图表、机器人大屏使用同一口径。全厂持有物料为在库与内部在途之和。
- 库房可对外出库，检验可直接发货：填写外部去向，从已接收库存预留，再由本班组一次确认整批；确认后锁定，不伪造内部下序或回流库存。
- 批量打印一份汇总单、一个条码，列出流水号、材质、类型、件数、重量和合计；常规批次一张 A4，内容过多自动续页，不删减明细。
