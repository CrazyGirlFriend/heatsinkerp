...
- 全厂总览下新增库存明细（`/factory-stock`）：八班组为行，完整材质名称及牌号为列，每格同时显示件数和 kg，含班组合计、材质合计及全厂总计。统计当前实物在库（含废料、不含在途），缺失班组标为未配置，不伪造零库存；全部材质均显示，超宽横向滚动。复用首页 SSE，在业务事务提交后更新并高亮变化，断线保留上次数据并重连，无定时轮询。此页需前后端同步发布，当前为本地新增功能。
