...
- 全厂总览下新增库存明细（`/factory-stock`）：八班组为行，完整材质名称及牌号为列，每格同时显示件数和 kg，含班组合计、材质合计及全厂总计。统计当前实物在库（含废料、不含在途），缺失班组标为未配置，不伪造零库存；全部材质均显示，超宽横向滚动。复用首页 SSE，在业务事务提交后更新并高亮变化，断线保留上次数据并重连，无定时轮询。已随 `20260922T203935Z` 前后端配套发布。
...
- 每班组包含库存明细、待接收、出库记录、丢失记录、材质归类和数据分析。流水号台账已合并至库存明细；已发布版本将八班组统一按流水号、来源、类型及材质规格分组，来源批次放在二级详情，行内出库限定所选分组，见 [分类库存说明](docs/team-classified-inventory.md)。图表单独展示。普通业务列表默认 10 条，可选 20/50/100 条并纵向滚动；单据详情与打印使用表格。当前视觉规则见 [界面一致性约定](docs/ui-style-guide.md)。
