...
- 每班组包含库存明细、待接收、出库记录、丢失记录、材质归类和数据分析。流水号台账已合并至库存明细，每行一个流水号，来源批次与收发、丢失记录在详情中查看；图表单独展示。普通业务列表默认 10 条，可选 20/50/100 条并纵向滚动；单据详情与打印使用表格。当前视觉规则见 [界面一致性约定](docs/ui-style-guide.md)。
